...
func get_respawn_position():
	...
	# If we have respawn points, get a random one and return it's global position
		...
		var respawn_point = randf_range(0, respawn_points.size()-1)
...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_cancel"):
		if popup == null:
			...
			# Connect the signals
			popup.get_node("Button_quit").connect("pressed", Callable(self, "popup_quit"))
			popup.connect("popup_hide", Callable(self, "popup_closed"))
			popup.get_node("Button_resume").connect("pressed", Callable(self, "popup_closed"))
